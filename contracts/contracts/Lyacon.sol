...
contract Lyacon is ERC20Burnable, Ownable {

    using SafeMath for uint256;

    uint256 public MAX_WALLET_STAKED = 10;
    uint256 public EMISSIONS_RATE = 11574070000000;
    uint256 public CLAIM_END_TIME = 1672574400;
    
    uint256 public TOTAL_BALANCE = 10000;
    address nullAddress = 0x0000000000000000000000000000000000000000;

    address public werewolvesAddress;
    
    //Mapping of WOLVES to timestamp
    mapping(uint256 => uint256) internal tokenIdToTimeStamp;

    //Mapping of WOLVES to staker
    mapping(uint256 => address) internal tokenIdToStaker;

    //Mapping of staker to WOLVES
    mapping(address => uint256[]) internal stakerToTokenIds;

    
    constructor() ERC20("LYACON", "Lyacon") {}

    function setWerewolvesAddress(address _werewolvesAddress) public onlyOwner {
        werewolvesAddress = _werewolvesAddress;
        return;
    }
    ...
    function stakeByIds(uint256[] memory tokenIds) public {
        require(
            stakerToTokenIds[msg.sender].length + tokenIds.length <=
                MAX_WALLET_STAKED,
            "Must have less than 10 WOLVES staked!"
        );

        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(
                IERC721(werewolvesAddress).ownerOf(tokenIds[i]) == msg.sender &&
                    tokenIdToStaker[tokenIds[i]] == nullAddress,
                "Token must be stakable by you!"
            );

            IERC721(werewolvesAddress).transferFrom(
                msg.sender,
                address(this),
                tokenIds[i]
            );
